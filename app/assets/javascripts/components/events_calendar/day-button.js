...
var Transition = require('material-ui/lib/styles/transitions');
...
var DayButton = React.createClass({
  ...
  componentWillReceiveProps: function componentWillReceiveProps(nextProps, nextContext) {
    ...
    this.setState({ muiTheme: newMuiTheme, all_events: this.state.all_events });
  },
  ...
  render: function render() {
    ...
    // tests if there's an event on each date of the month
    if (this.props.all_events && this.props.date) {
      var string = this.props.date.toJSON();
      var string_date = string.slice(0, string.indexOf("T"));
      var all_events = this.props.all_events;
      for (var key in all_events) {
        if (all_events[key].date == string_date) {
          styles.label.color = this.getTheme().color;
        }
      }
    }
